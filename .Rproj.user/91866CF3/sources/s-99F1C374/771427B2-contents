FitAbrupt <- function(x, mu, sig, fix.sh, sh.se, para=c("break", "single", "s")) {
  ...
  single <- 1:length(x)
  double_mu <- CutBreakpoint(1:length(x), mu, TRUE)
  double_sig <- CutBreakpoint(1:length(x), sig, TRUE)
  ...
  for (i in rank) {
    ...
    fit <- try(gev.fix(x, y, mul = mul, sigl = sigl, siglink = exp, siginit = siginit, fix.sh=fix.sh, show = FALSE),silent=TRUE)
    if('try-error' %in% class(fit)){
      ...
      fit <- gev.fix(x, y, mul = mul, sigl = sigl, siglink = exp, siginit = siginit, fix.sh=fix.sh, show = FALSE)
